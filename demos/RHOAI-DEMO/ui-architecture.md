...
Authentication is handled by a separate OAuth 2.1 authorization service that issues short-lived JWTs. Agents exchange API keys for tokens via the `client_credentials` grant; the MCP server validates JWTs using FastMCP's `JWTVerifier`. See [governance.md](../../docs/design/governance.md) for the full architecture.

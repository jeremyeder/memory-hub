...
Supports SSE streaming in the OpenAI format so the gateway and UI get
tokens as they arrive.

The fips-agents OpenAIChatServer doesn't support platform mode yet
(fips-agents/agent-template#231). This server is the workaround.
"""
...
async def chat_completions(request: ChatRequest):
    ...
    if request.stream:
        return StreamingResponse(
            _stream_response(agent, input_text, instructions, cmpl_id, request.model),
...
async def _stream_response(agent, input_text, instructions, cmpl_id, model):
    created = int(time.time())
